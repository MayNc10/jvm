...
impl PackedBoolArray {
    pub fn new(size: usize) -> PackedBoolArray {
        PackedBoolArray {
            m_array: Vec::with_capacity(((size - 1) / 8) + 1),
            m_true_size: size
        }
    }
    ...
    pub fn set(&mut self, index: usize, value: bool) {
        self.m_array[index / 8] |= (value as u8) << (index % 8); // 255 = 0b11111111, aka an u8 with all bits set.
    }
}
...
impl PackedBoolArray {
    pub fn get(&self, index: usize) -> bool {
        if index >= self.m_true_size {
            panic!("Index out of range for PackedBoolArray! Index: {index}, Size: {}", self.m_true_size);
        }
        (self.m_array[index / 8] & 1 << (index % 8)) != 0
    }
}
