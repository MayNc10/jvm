...
impl System {
    ...
    pub fn set_security_manager(&mut self, jvm: &mut JVM, _obj_ref: Reference<dyn Class, dyn Object>) -> Result<(), Error> {
        ...
        let e_obj = object::new_object_with_name("java/lang/UnsupportedOperationException", jvm)?;
    ...
    {   
        ...
        if let Reference::Null = src_ref {
            e_obj = object::new_object_with_name("java/lang/NullPointerException", jvm)?;
        }
            ...
            (src_pos + len) as usize > src.len() || (dest_pos + len) as usize > dest.len() {
                e_obj = object::new_object_with_name("java/lang/IndexOutOfBoundsException", jvm)?;
            }
            else {
            ...
            e_obj = object::new_object_with_name("java/lang/ArrayStoreException", jvm)?;  
        }
        let e_ref = Reference::<dyn Class, dyn Object>::Object(e_obj, Rc::new(Monitor::new()));
        let thread = current_thread_mut!(jvm);
        let frame = current_frame_mut!(thread);
...
impl Class for System {
    fn new(file: classfile::ClassFile, jvm: &mut JVM) -> Result<Self, Error> where Self : Sized {
        let out_object = natives::print_stream::PrintStream::new_stdout(jvm)?;
